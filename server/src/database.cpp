...
void Database::add_info_user(const User_info& client) {
  ...
  for (size_t i = 0; i < WEATHER_ARR_SIZE; ++i) {
    std::string sql_request{
        ...
        fmt::format(
            "('{}', {}, '{}', '{}', {:.4f}, {:.4f}, {:.4f}, {:.4f}, {});",
            client.name, client.city, city_weather.arr[i].date.date,
            city_weather.arr[i].date.time, city_weather.arr[i].temp,
            city_weather.arr[i].temp_feels_like, city_weather.arr[i].pressure,
            city_weather.arr[i].wind, city_weather.arr[i].type)};
...
void Database::change_city(const User_info& client) {
  std::string sql_request{
      "update weather "
      "set city = " +
      fmt::format("{}", client.city) + " where user_name = '" + client.name +
      "';"};
