...
size_t window_show_city( User_city_t user_city )
{
    ...
    switch (user_city)
    {
    case MOSCOW:        city_name = "Moscow";          break;
    case STPETESBURG:   city_name = "StPetersburg";    break;
    case NOVOSIBIRSK:   city_name = "Novosibirsk";     break;
    case YEKATERINBURG: city_name = "Yekaterinburg";   break;
    case DOLOGOPRUDNY:  city_name = "Dolgoprudny";     break;
    default: break;
    }
    ...
    init_text(text_city_name, 700, 300, city_name, City_Ftext);     // 7000, 50 - позиция
  ...
  //рисуем рамку
    ...
    init_text(question_change_city, 400, 450, "Change your city?", Ftext);     // 7000, 50 - позиция

    // Название пунктов меню
    std::vector<std::string> name_menu{ "Yes","No" };

    // Создаем объект меню
    app::app_menu my_menu(window,150, 600, 100, 120, name_menu); //окно, координаты, размер шрифта, шаг, массив названий
        ...
        while (window.pollEvent(event))
        {
            if (event.type == Event::KeyReleased)
            {
                ...
                if (event.key.code == Keyboard::Return)                         // ввод
                {
                    ...
                    switch (my_menu.get_selected_menu_number())
                    {
                    case 0: return NEED_CHANGE_CITY;       break; 
                    case 1: return NO_NEED_CHANGE_CITY;    break;
                    default:break;
                    }
                }
