...
const std::string PATH_IMAGE_1 = "../front_end/res/image/IMG_2439.JPG";
...
const std::string PATH_PIC_RAIN = "../front_end/res/image/IMG_2467.png";

//------------------------------------------------------------------------------------------------------

std::string name_weather_pic(Weather_t weather_type) {
  if (weather_type == Clear_sky)
    return PATH_PIC_SUN;
  else if (weather_type == Rain)
    return PATH_PIC_RAIN;
  else
    return PATH_PIC_CLOUD;
}
...
sf::Sprite show_pic_weather(sf::Texture texture_pic, sf::Sprite sprite_pic,
  ...
  if (!texture_pic.loadFromFile(pic_name)) {
...
void weath_today(const std::vector<Weather_info>& arr) {
  ...
  std::string info2 = "" + arr[0].date.date + "\n" +
  ...
  if (!texture1.loadFromFile(path_pic1)) {
    std::cout << "Could not load enemy texture" << std::endl;
  }
  sf::Sprite sprite1;
  sprite1.setTexture(texture1);
  sprite1.setPosition(sf::Vector2f(1 * (step_x - 40) + start_x + 270,
                                   4 * (step_y + 22) + start_y - 77));
  ...
  while (nday_window.isOpen()) {
    ...
    for (int i = 0; i < columns; i++) {
      for (int j = 0; j < rows; j++) {
        ...
        grid_g[i][j].setPosition(i * step_x + start_x - 20,
                                 j * step_y + start_y);
        ...
        if (i > 0) {
          ...
          grid_v[i][j].setPosition(i * (step_x - 40) + start_x + 200,
                                   j * (step_y + 26) + start_y - 77);
...
void weath_tomorrow(const std::vector<Weather_info>& arr) {
  ...
  std::string info3 = "" + arr[1].date.date + "\n" +
  ...
  if (!texture2.loadFromFile(path_pic2)) {
    std::cout << "Could not load enemy texture" << std::endl;
  }
  sf::Sprite sprite2;
  sprite2.setTexture(texture2);
  sprite2.setPosition(sf::Vector2f(1 * (step_x - 40) + start_x + 270,
                                   4 * (step_y + 22) + start_y - 77));
...
void weath_3days(const std::vector<Weather_info>& arr) {
  ...
  std::string info4 = "" + arr[2].date.date + "\n" +
  ...
  sf::Sprite sprite2;
  sprite2.setTexture(texture2);
  sprite2.setPosition(sf::Vector2f(2 * (step_x - 40) + start_x + 270,
                                   4 * (step_y + 22) + start_y - 77));
  sprite2.scale(sf::Vector2f(0.5, 0.5));

  sf::Texture texture3;
  std::string path_pic3 = name_weather_pic(arr[2].type);
  if (!texture3.loadFromFile(path_pic3)) {
    std::cout << "Could not load enemy texture" << std::endl;
  }
  sf::Sprite sprite3;
  sprite3.setTexture(texture3);
  sprite3.setPosition(sf::Vector2f(3 * (step_x - 40) + start_x + 270,
                                   4 * (step_y + 22) + start_y - 77));
...
void weath_5days(const std::vector<Weather_info>& arr) {
  ...
  std::string info5 = "" + arr[3].date.date + "\n" +
  ...
  std::string info6 = "" + arr[4].date.date + "\n" +
  ...
  if (!texture4.loadFromFile(path_pic4)) {
    std::cout << "Could not load enemy texture" << std::endl;
  }
  sf::Sprite sprite4;
  sprite4.setTexture(texture4);
  sprite4.setPosition(sf::Vector2f(4 * (step_x - 40) + start_x + 270,
                                   4 * (step_y + 22) + start_y - 77));
  sprite4.scale(sf::Vector2f(0.5, 0.5));

  sf::Texture texture5;
  std::string path_pic5 = name_weather_pic(arr[4].type);
  if (!texture5.loadFromFile(path_pic5)) {
    std::cout << "Could not load enemy texture" << std::endl;
  }
  sf::Sprite sprite5;
  sprite5.setTexture(texture5);
  sprite5.setPosition(sf::Vector2f(5 * (step_x - 40) + start_x + 270,
                                   4 * (step_y + 22) + start_y - 77));
