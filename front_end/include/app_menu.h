...
private:
	...
	void set_init_text(sf::Text& text, const std::string& str, float xpos, float ypos) const;

public:

														//конструктор
	app_menu(sf::RenderWindow& window, float menux, float menuy, int sizeFont, int step, std::vector<std::string>& name);		  
...
//------------------------------------------------------------------------------------------------------
	...
	void set_color_text_menu(sf::Color menColor, sf::Color ChoColor, sf::Color BordColor);
