...
void HTTP::Parsing::get_buffer(double lat, double lon) {
    CURL *curl;
    CURLcode res;
    std::string api_request = "api.openweathermap.org/data/2.5/forecast?lat=55.939831&lon=37.503434&exclude=daily&current.temp=Celsius&units=metric&appid=464a485473994b656c2f1419b57a028a";
    
    // "https://api.openweathermap.org/data/2.5/forecast?lat=" + std::to_string(lat) + "&lon=" + std::to_string(lon)
    ...
    if (curl) {
        ...
        if (res == CURLE_OK)  std::cout << buffer << std::endl; 
    }
}
